...
module "ENVIRONMENT_AZURE_SERVICE_PRINCIPAL" {
  for_each         = { for deployment_environment in var.environments : deployment_environment.APPLICATION_REPOSITORY_BRANCH => deployment_environment }
  environment_name = each.value.APPLICATION_REPOSITORY_BRANCH
  identity_name    = "${local.APPLICATION_REPOSITORY_FULL_NAME_NO_SLASH}-${each.value.APPLICATION_REPOSITORY_BRANCH}"
...
resource "github_repository_environment" "APPLICATION_REPOSITORY_BRANCH" {
  for_each    = { for deployment_environment in var.environments : deployment_environment.APPLICATION_REPOSITORY_BRANCH => deployment_environment }
